...
const ELON_GOOD_QUOTES = [
  ...
  "You're cooking with rocket fuel now.",
  "S-tier content right here.",
  "The simulation approves.",
  "Viral potential: extremely high.",
  "You might actually make it.",
  "My timeline needs more of this.",
  "First principles thinking. I like it.",
  "The meme magic is real.",
  "Absolutely gigabased.",
  "You understand the game.",
  "This will print engagement.",
  "Chef's kiss. No notes.",
  "Twitter University valedictorian.",
  "The ratio gods are pleased.",
];
...
const ELON_BAD_QUOTES = [
  ...
  "So this is all you got done this week?",
  "Even my bots write better.",
  "The NPCs are not impressed.",
  "Did an intern write this?",
  "My Mars colonists would be disappointed.",
  "This is why AI will replace us.",
  "Ratio incoming in 3... 2... 1...",
  "The algorithm is crying.",
  "You're better than this. Maybe.",
  "I've seen better takes from politicians.",
  "Even Twitter's old bird logo is judging you.",
  "The cringe-to-engagement ratio is off.",
  "This needs a complete redesign.",
  "Ctrl+Z this immediately.",
  "Not even Grok can save this.",
  "The timeline didn't need this.",
  "My 11 kids could do better.",
  "This is why I bought Twitter.",
  "Peak NPC energy detected.",
  "The simulation is disappointed.",
];
...
const TRUMP_QUOTES = [
  ...
  "PERFECT replies! Absolutely perfect!",
  "This is YUGE! Really, really yuge!",
  "AMAZING work! The likes of which nobody has ever seen!",
  "You're doing BIGLY! So bigly!",
  "SPECTACULAR! The best I've ever seen!",
  "PHENOMENAL replies! Just phenomenal!",
  "This is what WINNING looks like!",
  "EXTRAORDINARY! Truly extraordinary!",
  "The BEST! Nobody does replies like you!",
  "MAGNIFICENT! Absolutely magnificent!",
];

// Reaction images
const ELON_IMAGES = {
  happy: '/images/elon/elon-happy.png',
  sad: '/images/elon/elon-sad.png',
  impressed: '/images/elon/elon-impressed.png',
  angry: '/images/elon/elon-angry.png',
  excited: '/images/elon/elon-excited.png',
  bored: '/images/elon/elon-bored.png',
};

const TRUMP_IMAGE = '/images/elon/trump-happy.png';
...
// Mascot component (Elon or Trump)
...
function Mascot({ mood, quote, visible }: { mood: MascotMood; quote: string; visible: boolean }) {
  ...
  const imageSrc = isTrump ? TRUMP_IMAGE : ELON_IMAGES[mood] || ELON_IMAGES.happy;
  ...
  return (
    <div style={{
      ...
      {/* Character image */}
      <div style={{
        width: isTrump ? 140 : 120,
        height: isTrump ? 140 : 120,
        borderRadius: 12,
        overflow: 'hidden',
...
// Duo mascot - Elon calls Trump, then Elon leaves and Trump appears alone
function DuoMascot({ elonQuote, trumpQuote, visible }: { elonQuote: string; trumpQuote: string; visible: boolean }) {
  const [phase, setPhase] = useState<'elon' | 'trump'>('elon');
  const typedElonQuote = useTypingEffect(visible && phase === 'elon' ? elonQuote : '', 25);
  const typedTrumpQuote = useTypingEffect(visible && phase === 'trump' ? trumpQuote : '', 30);
  ...
  useEffect(() => {
    ...
    // After Elon finishes typing, he leaves and Trump enters
    const elonDuration = elonQuote.length * 25 + 1000;
    const timer1 = setTimeout(() => setPhase('trump'), elonDuration);
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ fontSize: 13, color: '#fff', lineHeight: 1.4, fontFamily: 'monospace', minHeight: 20 }}>
            {typedElonQuote}
            <span style={{ opacity: typedElonQuote.length < elonQuote.length ? 1 : 0, animation: 'blink 0.5s infinite' }}>|</span>
          </div>
          <div style={{
        ...
        <div style={{ width: 120, height: 120, borderRadius: 12, overflow: 'hidden', animation: 'elonBounce 0.5s ease' }}>
          <img src={ELON_IMAGES.excited} alt="Elon" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        </div>
      </div>
    );
  }

  // Phase 2: Trump appears (Elon is gone)
  return (
    <div style={{
      position: 'fixed',
      bottom: 80,
      ...
      alignItems: 'flex-end',
      gap: 12,
      animation: 'trumpEntrance 0.5s ease',
      zIndex: 100
    }}>
      <div style={{
        background: '#1a1a2a',
        ...
        <div style={{ fontSize: 14, color: '#fff', lineHeight: 1.4, fontFamily: 'monospace', minHeight: 20, fontWeight: 500 }}>
          {typedTrumpQuote}
          <span style={{ opacity: typedTrumpQuote.length < trumpQuote.length ? 1 : 0, animation: 'blink 0.5s infinite' }}>|</span>
        </div>
        <div style={{
          position: 'absolute', bottom: -8, right: 30,
          width: 0, height: 0,
      ...
      <div style={{
        ...
        <img src={TRUMP_IMAGE} alt="Trump" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
      </div>
...
// Compact tweet embed for context (original tweet only, no conversation)
function OriginalTweetEmbed({ url }: { url: string }) {
  ...
  // hideCard=true hides link previews, hideThread=true hides replies
...
export default function ReplyGuySchool() {
  ...
  const [showBothMascots, setShowBothMascots] = useState(false);
  ...
  async function handleAction(rating: 'good' | 'bad' | 'skip') {
    ...
    setShowBothMascots(false);

    // Calculate streak IMMEDIATELY (before any async)
    let mood: MascotMood;
    let quote: string;
    let shouldShowTrump = false;
    let trumpQ = '';

    if (rating === 'good') {
      goodStreakRef.current += 1;
      const newStreak = goodStreakRef.current;
      console.log('Good streak:', newStreak);

      // Streak logic: Every 3rd good, Trump appears
      if (newStreak % 3 === 0) {
        mood = 'excited';
        quote = "Don! Get over here, you gotta see this!";
        // First Trump (3rd good) always says TERRIFIC
        if (newStreak === 3) {
          trumpQ = "TERRIFIC! This is absolutely TERRIFIC work!";
        } else {
          trumpQ = TRUMP_QUOTES[Math.floor(Math.random() * TRUMP_QUOTES.length)];
        }
        shouldShowTrump = true;
        console.log('TRUMP TIME! Streak:', newStreak);
      } else {
      ...
      goodStreakRef.current = 0; // Reset streak
      ...
      // Skip - don't reset streak, show bored Elon
      mood = 'bored';
    ...
    setTimeout(() => {
      ...
      setShowBothMascots(shouldShowTrump);
      if (shouldShowTrump) {
        setTrumpQuote(trumpQ);
      }
    ...
    <div style={{ background: '#000', color: '#fff', minHeight: '100vh', fontFamily: 'system-ui' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 24px', borderBottom: '1px solid #222' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={copyPrompt} style={{ background: 'none', color: copied ? '#fff' : '#666', border: '1px solid #333', padding: '6px 12px', borderRadius: 4, fontSize: 12, cursor: 'pointer' }}>
            {copied ? 'Copied' : 'Copy Prompt to improve the reply agent'}
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 120px)', padding: '20px', position: 'relative' }}>
        ...
        {loading ? (
          ...
          >
            {/* OUR REPLY - prominently displayed first */}
            <div style={{
              background: '#111',
              border: '1px solid #333',
              borderRadius: 12,
            ...
            {/* ORIGINAL TWEET - embedded (only if we have the actual original tweet URL) */}
            ...
            {/* If original tweet not available, show reply URL link */}
            {(!current.tweet_url || current.tweet_url === current.reply_url) && current.reply_url && (
              <div style={{ marginBottom: 12, fontSize: 11, color: '#444' }}>
                <a href={current.reply_url} target="_blank" rel="noopener noreferrer" style={{ color: '#555' }}>
                  View reply on X (original tweet not captured)
                </a>
              </div>
            )}
      ...
      {/* Mascot (Elon, or Elon + Trump together) */}
      {showBothMascots ? (
        <DuoMascot
          elonQuote={mascotQuote}
